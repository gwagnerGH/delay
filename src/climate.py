...
from .config import get_base_year_reference
...
class BPWClimate(Climate):
    ...
    The following climate parameters:
        ...
        C_BASE_YEAR: float
            CO2 concentrations in the model base year (in ppm)
        F_0: float
    ...
    def __init__(self, tree, emit_baseline, draws):
        ...
        if self.base_year_reference.co2_concentration is None:
            raise ValueError(f"Missing CO2 concentration reference for base year {self.base_year}.")

        # forcing equation constants
        self.C_0 = 278 # ppm
        self.F_0 = 5.35 # W m^-2
        self.C_BASE_YEAR = self.base_year_reference.co2_concentration
    ...
    def get_conc_and_forcing_at_node(self, m, node, returning="both",
        ...
        if node == 0:
            if returning == "conc":
                return self.C_BASE_YEAR
            elif returning == "forcing":
                return 0.0
            elif returning == "both":
                return self.C_BASE_YEAR, 0.0
        ...
        FINAL_TIME = trunc_times[-1] - self.base_year
        time = np.arange(-FINAL_TIME, FINAL_TIME, 1)

        # parse time range
        time_after_base_year = time[FINAL_TIME:]
        time_before_base_year = time[:FINAL_TIME]

        # now interpolate bau pathway from t = 0 to t = FINAL_TIME
        interp_bau = np.interp(time_after_base_year, trunc_times - self.base_year,
                               mitigated_baseline)

        # make emission pathway zero before the base year and emission_path after the base year
        # the "half zeros" are required to get a consistent convolution
        # with the IRF below
        bau_interp_full_path = np.hstack((np.zeros_like(time_before_base_year), interp_bau))

        # make Joos et al 2013 IRF with zeros before the base year as was done for emission path
        joos_IRF = self._make_joos_IRF(time_after_base_year, time_before_base_year)
        ...
        preindustrial_post_base_year = self.C_BASE_YEAR * (self.A_0 + self.A_1)**(-1) * \
            (self.A_0 + self.A_1 * np.exp(-time_after_base_year / self.TAU_1))
        total_preindustrial = np.hstack((np.zeros(FINAL_TIME),
                                         preindustrial_post_base_year))
