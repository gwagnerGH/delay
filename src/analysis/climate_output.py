...
from ..config import DEFAULT_BASE_YEAR
...
class ClimateOutput(object):
    ...
    def calculate_output(self, m):
        ...
        for period in range(0, periods):
            ...
            for node in range(nodes[0], nodes[1]+1):
                # Use the damage-side definition for average mitigation to ensure
                # consistent time weighting and exclusion of the current node’s action
                mean_mitigation = self.utility.damage.average_mitigation_node(m, node, period)
                price = self.utility.cost.price(years, m[node], mean_mitigation)
                self.prices[node] = price
                self.ave_mitigations[node] = mean_mitigation
            ...
            # Expected emissions for the period: BAU emissions at this decision time
            # multiplied by expected (1 - raw mitigation) across nodes in the period
            baseline = emit_baseline.baseline_gtco2_periods[period]
            node_slice = slice(nodes[0], nodes[1]+1)
            self.expected_period_emissions[period] = baseline * np.dot(1.0 - m[node_slice], probs)
    ...
    def _store_trees(self, prefix=None, start_year=None, tree_dict={}):
        ...
        **kwargs
        ...
        if start_year is None:
            start_year = getattr(self.utility.tree, "base_year", DEFAULT_BASE_YEAR)
