...
class DamageSimulation(object):
    ...
    def _run_path(self, const_mitigation_val):
        ...
        # point. but for the simulation, we only want the damages past the base
        # year, so cut off the first value.
        damage_after_start = damage[:, 1:]

        # sort damages based on outcome of simulation
        damage_sorted = self._sort_array(damage_after_start)
    ...
    def _damage_function_stat(self, temp_pathways, MC):
        ...
        dec = np.asarray(self.tree.decision_times)
        years = dec if dec.min() >= 1900 else (self.tree.base_year + dec)

        split_year = 2100
        mid_mask = (years < split_year)
        end_mask = ~mid_mask 

        damage[:, mid_mask] = temp_pathways[:, mid_mask] * (
            b2_mid[:, None] * temp_pathways[:, mid_mask] + b1_mid[:, None]
        )

        damage[:, end_mask] = temp_pathways[:, end_mask] * (
            b2_end[:, None] * temp_pathways[:, end_mask] + b1_end[:, None]
        )
