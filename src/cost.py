...
class BPWCost(Cost):
    ...
    def price(self, years, mitigation, ave_mitigation):
        ...
        if np.any(over_mit):
            premium_price = tech_term * (self.tau_0 + self.backstop_premium) \
                            * (np.exp(mitigation*self.power) - 1)
            if np.ndim(mitigation) == 0:
                price = premium_price
            else:
                price = np.where(over_mit, premium_price, price)
